%% IK-MOD.m
function [DictionaryNew, output] = IKMOD_rms_new4(DataNew, DataOld, DictionaryOld, param, coding)

%% subfunction Method.A
...
% on "ksvd-learning-v4" by Dr.Azimi. ( FINAL VERSION )
...
%% ---- (2.1) Compute the error of sparse representation of all of the samples by DictionaryOld as the first element of the errors to be monitored.
CoefMatrixDataAll = OMP(DictionaryOld,[DataOld DataNew0], coding.L, coding.errorGoal);
VecError_Old = DataOld-DictionaryOld*CoefMatrixDataAll(:,1:size(DataOld,2));
...
% (2.1) *** update dictionary one atom at a time ***
output.dk_update = [];
while ResError_diff>ResError_Old*0.0 && loop_num<param.K1 % && ResError_diff_all>0.000 %(isempty(DataNew)==0) % && abs(ResError(end)-ResError(end-1))>0.05*coding.errorGoal)
    
    loop_num = loop_num+1;
    
    %% (2.3.1) find the coefficients of new samples using sparse coding in the subspace spanned by DitionaryOld.
    CoefMatrixDataNew = OMP(DictionaryOld,DataNew, coding.L-1*1, coding.errorGoal);
    r_i_old = DataNew-DictionaryOld*CoefMatrixDataNew; % the residual error matrix of the new data and its sparse representation by DictionaryOld
    ...
    for iterNum_K = 1:param.numIteration
        ...
        r_i_old = DataNew-DictionaryOld*CoefMatrixDataNew(1:end-1,:); % the residual error matrix of the new data and its sparse representation by DictionaryOld
        idx_nonzero = find(CoefVector_DataNewAtom~=0);
        CoefVector_DataNewAtom_R = CoefVector_DataNewAtom(idx_nonzero);
        r_i_old_R = r_i_old(:,idx_nonzero);
        ...
        while iterNum<10 && ResError3_Data>param.MOD_Err && AtomError_diff12>param.MOD_diffErr
            ...
            NewAtom0 = d_new;
            d_new = (r_i_old_R*CoefVector_DataNewAtom_R')/sqrt(CoefVector_DataNewAtom_R*(r_i_old_R')*r_i_old_R*CoefVector_DataNewAtom_R');
            CoefVector_DataNewAtom_R = d_new'*r_i_old_R; % the coefficients of the new samples cooresponding to the new atom added.
            
            CoefVector_DataNewAtom = zeros(1,size(DataNew,2));
            CoefVector_DataNewAtom(idx_nonzero) = CoefVector_DataNewAtom_R;
            ResErrorMatrix_Data = r_i_old-d_new*CoefVector_DataNewAtom;
        ...
        output.totalerr(iterNum_K) = sqrt(sum(sum(([DataOld DataNew0]-DictionaryTemp*CoefMatrix).^2)) / size([DataOld DataNew0], 2));
        output.totalerrNew(iterNum_K) = sqrt(sum(sum((DataNew0-DictionaryTemp*CoefMatrix(:,size(DataOld,2)+1:end)).^2)) / size(DataNew0, 2));
        output.totalerrOld(iterNum_K) = sqrt(sum(sum((DataOld-DictionaryTemp*CoefMatrix(:,1:size(DataOld,2))).^2)) / size(DataOld, 2));
        output.dk_update = [output.dk_update d_new];
        if (iterNum_K<3) continue; end;
        if strcmp(coding.method, 'MP')
            if ~coding.errorFlag
                if (output.numCoef(iterNum_K)<=coding.L || abs(output.totalerr(iterNum_K)-output.totalerr(iterNum_K-1))<0.01*coding.errorGoal)
                    output.CoefMatrix = CoefMatrix;
                    break ;
                end;
            else
                if (output.totalerr(iterNum_K)<=coding.errorGoal  || abs(output.totalerr(iterNum_K)-output.totalerr(iterNum_K-1))<0.01*coding.errorGoal)
                    output.CoefMatrix = CoefMatrix;
                    break;
                end;
            end
