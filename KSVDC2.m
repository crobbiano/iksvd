function [Dictionary,output] = KSVDC2(...
    Data,... % an nXN matrix that contins N signals (Y), each of dimension n.
    param,thr)

% MODIFIED K-SVD IMPLEMENTATION THAT WORKS FOR BOTH REAL-VALUED AND COMPLEX-VALUED TRAINING SIGNALS.
...
% =========================================================================
% INPUT ARGUMENTS:
% thr                          Error threshold used within sparse coding (along with fixed sparsity level)
% Data                         an nXN matrix that contins N signals (Y), each of dimension n. 
...
% coefficient calculation method is OMP with fixed number of coefficients, error threshold
...
for iterNum = 1:param.numIteration
    ...
    if (param.errorFlag==0)
        ...
        CoefMatrix = OMPerrn([FixedDictionaryElement,Dictionary],Data,thr,param.L);
    ...
    if (iterNum>1 & param.displayProgress)
        if (param.errorFlag==0)
            output.totalerr(iterNum-1) = sqrt(sum(sum((abs(Data-[FixedDictionaryElement,Dictionary]*CoefMatrix)).^2))/prod(size(Data)));
    ...
    Dictionary = I_clearDictionary([FixedDictionaryElement,Dictionary],CoefMatrix,Data);
...
% first, all the column in oiginal starts with positive values (just the real parts).
catchCounter = 0;
totalDistances = 0;
for i = 1:size(new,2)
    new(:,i) = sign(real(new(1,i)))*new(:,i);
end
for i = 1:size(original,2)
    d = sign(real(original(1,i)))*original(:,i);
    distances =sum ( (abs(new-repmat(d,1,size(new,2)))).^2);
